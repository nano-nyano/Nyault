...
export class WalletService {
        ...
        if (shouldNotify || transaction.block.subtype === 'change') {
          ...
          if (transaction.block.subtype === 'send') {
            ...
            if (this.addressBook.getTransactionTrackingById(addressLink)) {
              this.notifications.sendInfo(`Tracked address ${accountHrefLink} can now receive ${trackedAmount} NYANO`, { length: 10000 });
              console.log(`Tracked incoming block to: ${address} - ${trackedAmount} Nyano`);
            }
            // Outgoing transaction
            if (this.addressBook.getTransactionTrackingById(address)) {
              this.notifications.sendInfo(`Tracked address ${accountHref} sent ${trackedAmount} NYANO`, { length: 10000 });
              console.log(`Tracked send block from: ${address} - ${trackedAmount} Nyano`);
            }
          } else if (transaction.block.subtype === 'receive' && this.addressBook.getTransactionTrackingById(address)) {
            // Receive transaction
            this.notifications.sendInfo(`Tracked address ${accountHref} received incoming ${trackedAmount} NYANO`, { length: 10000 });
            console.log(`Tracked receive block to: ${address} - ${trackedAmount} Nyano`);
  ...
  async processPendingBlocks() {
    ...
    if (newHash) {
      ...
      this.notifications.sendSuccess(`Successfully received ${receiveAmount.isZero() ? '' : receiveAmount.toFixed(6)} Nyano!`, { identifier: 'success-receive' });
