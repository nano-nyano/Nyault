...
export class SweeperComponent implements OnInit {
  ...
  async processSend(privKey, previous, sendCallback) {
    ...
    if (this.validDestination && nanocurrency.checkAddress(this.destinationAccount)) {
      ...
      if (data.hash) {
        ...
        (nanoAmountSent ? (nanoAmountSent.toString(10) + ' Nyano') : '') + ' transferred to ' + this.destinationAccount, {length: 15000});
        this.appendLog('Funds transferred ' + (nanoAmountSent ? ('(' + nanoAmountSent.toString(10) + ' Nyano)') : '') + ': ' + data.hash);
  ...
  async createPendingBlocks(privKey, address, balance, previous, subType, callback, accountCallback) {
    ...
    if (data.blocks) {
      ...
      const pending = {count: Object.keys(data.blocks).length, raw: raw, NYANO: nanoAmount, blocks: data.blocks};
      const row = 'Found ' + pending.count + ' pending containing total ' + pending.NYANO + ' NYANO';
      this.appendLog(row);
  ...
  async processIndexRecursive(privKeys, keyCount) {
    ...
    this.processAccount(privKey, function() {
      ...
      if (keyCount < privKeys.length) {
        ...
        this.appendLog(this.totalSwept + ' Nyano transferred');
        this.notificationService.sendInfo('Finished processing all accounts. ' + this.totalSwept + ' Nyano transferred', {length: 0});
        this.sweeping = false;
      }
    }.bind(this));
  }
