...
export class AccountDetailsComponent implements OnInit, OnDestroy {
  ...
  remoteVisible = false;
  blockTypes: string[] = ['Send Nyano', 'Change Representative'];
  ...
  async receiveReceivableBlock(receivableBlock) {
    ...
    if (createdReceiveBlockHash) {
      ...
      this.notifications.sendSuccess(`Successfully received Nyano!`, { identifier: 'success-receive' });
  ...
  async generateSend() {
    ...
    if (!this.validateAmount()) return this.notifications.sendWarning(`Invalid NYANO Amount`);
