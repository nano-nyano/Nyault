...
export class SendComponent implements OnInit {
  ...
  amounts = [
    { name: 'NYANO', shortName: 'NYANO', value: 'nyano' }
  ];
  ...
  async validateDestination() {
    ...
    if (!this.addressBookMatch && this.toAccountID === environment.donationAddress) {
      this.addressBookMatch = 'NyanoWallet Donations';
    }
  ...
  async sendTransaction() {
    ...
    if (!this.validateAmount()) {
      return this.notificationService.sendWarning(`Invalid NYANO Amount`);
    }
    ...
    if (from.balanceBN.minus(rawAmount).lessThan(0)) {
      return this.notificationService.sendError(`From account does not have enough NYANO`);
    }
  ...
  setMaxAmount() {
    ...
    const nanoVal = this.util.nano.rawToMnano(walletAccount.balance).floor();
